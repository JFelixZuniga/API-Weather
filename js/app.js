...
function buscarClima(e) {
  ...
  if (ciudad === "") {
    // Hubo un error
    mostrarError("Debe ingresar una ciudad o país");
...
function consultarAPI(ciudad) {
  ...
  let url = `http://api.openweathermap.org/data/2.5/weather?q=${ciudad}&appid=${appId}`;
  ...
  fetch(url)
    ...
    .then((datos) => {
      ...
      limpiarHTML();
      if (datos.cod === "404") {
        mostrarError("Ciudad o País No Encontrado");
      } else {
        mostrarClima(datos);
      }
    })
